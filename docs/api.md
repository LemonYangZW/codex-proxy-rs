...
OpenAI 的 `turnStateBytes` 是上游 `x-codex-turn-state` 令牌 base64 解码后的字节数（`null` 表示无令牌、
无法解码或时间戳不可信），令牌本体不落库。该令牌是 Fernet 封装：0x80 版本字节 + 8 字节大端秒级
签名时刻 + 16 字节 IV + AES-CBC 密文 + 32 字节 HMAC。Provider 观测另保留 `turnStateVersion`、
`turnStateIssuedAtUnix` / `turnStateIssuedAt` 供诊断；时间戳是签名时刻，实测有效期约 1 小时，
不是上游声明的过期时间。管理端按密文块数判定：个人号正常 10 块（base64 292 字符）、
team 号正常 12 块（332 字符），疑似降智在各自基线上恰好多一块（312 / 356 字符）；
体积可解析但不在正常形态内记「疑似降智」，读不出 Fernet 结构或本轮无令牌记「无法判定」。
PKCS7 填充下块数只能把明文框进 16 字节窗口，因此这是疑似判据而非确证，也不影响路由、计费或账号状态。
上游只在部分轮次下发该令牌，缺失时不由历史长度补齐；历史记录不做回填。
